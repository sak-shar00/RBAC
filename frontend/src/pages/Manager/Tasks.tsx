  ...
  return (
    <Layout>
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => { setEditingTask(null); setFormData({ title: "", description: "", project: "", assignedTo: "" }); setShowModal(true); }}>
          Create Task
        </Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid gap-4">
          {tasks.map((task) => (
            <div key={task._id} className="bg-white p-4 rounded shadow hover:shadow-md transition-shadow">
              <div className="flex justify-between items-start">
                ...
                <div className="flex gap-2 ml-4">
                  <Button variant="outline" size="sm" onClick={() => setAssignModal(task._id)}>
                    Assign
                  </Button>
      ...
      {assignModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg w-96">
            ...
            <p className="mb-4 text-gray-600">Select a developer to assign this task:</p>
            {developers.length === 0 ? (
              <p className="text-gray-500">No developers available</p>
            ) : (
              <div className="flex flex-col gap-2 max-h-60 overflow-y-auto">
                {developers.map((dev) => (
                  <Button key={dev._id} variant="outline" onClick={() => handleAssign(assignModal, dev._id)}>
                    {dev.name} ({dev.email})
                  </Button>
                ))}
              </div>
            )}
            <Button type="button" variant="ghost" className="mt-4" onClick={() => setAssignModal(null)}>
              Cancel
            </Button>
          </div>
        </div>
